...
Local LLM과 한국투자증권 KIS API를 이용하는 단일 종목 단기 자동매매 시스템.
...
- 본 소프트웨어는 알고리즘 트레이딩 및 인공지능 연구·학습 목적으로 개발된 오픈소스 프로그램입니다.
- 금융 시장에서의 투자 및 거래는 항상 원금 손실의 위험을 수반합니다. 본 프로그램이 제공하는 분석 결과, 매매 신호, 자동 주문 기능 등은 특정 종목에 대한 투자 권유나 수익을 보장하지 않습니다.
- 프로그램의 실행, 오작동, 네트워크 지연, 증권사 API 장애 또는 AI 모델의 오판으로 인해 발생하는 모든 직접적·간접적 투자 손실에 대한 최종 책임은 전적으로 사용자 본인에게 있습니다.
- 실계좌 적용 전 반드시 모의투자 환경(`KIS_IS_PAPER=true`)에서 충분한 검증을 거친 후 사용하시기 바랍니다.
