...
# ── 장 운영 시간 ─────────────────────────────────────
# KIS 1분봉 API(FHKST03010200)는 정규장(09:00~15:30) 데이터만 반환한다.
# 한국 시간외 거래는 3단계로 나뉘는데:
# 구분	            시간	        KIS API	        1분봉 제공
# 정규장	        09:00~15:30	    FHKST03010200	O
# 시간외 종가매매	15:40~16:00	    별도 API	    X (단일가 체결)
# 시간외 단일가	    16:00~18:00	    별도 API	    X (10분 단위 단일가)
#
# 시간외 거래는 단일가 경매 방식이라 1분봉 형태의 연속 데이터가 존재하지 않는다.
# 기술 지표(RSI, MACD, BB) 분석이 의미가 없으므로, 정규장에서만 동작한다.
#
...
MAX_TRADES_PER_DAY: int = int(
    os.environ.get("MAX_TRADES_PER_DAY", "5")
)  # 일일 최대 거래 횟수
