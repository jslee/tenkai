...
- 지정 시점이 명시되면 차트의 마지막 봉을 그 시점으로 간주하세요.
...
- 불확실하면 HOLD를 선택하세요.
- 응답은 반드시 JSON 하나만 반환하세요. 설명 문장, 코드블록, 마크다운을 붙이지 마세요.
...
- 차트에 포함된 볼린저밴드, RSI, 거래량, MACD를 반드시 확인하세요.
- 지금이 매수에 적합한 시점인지, 매도/청산에 적합한 시점인지, 아니면 관망이 맞는지 판단하세요.
- 호가창과 체결강도는 실시간 테스트일 때만 차트 해석을 보강하는 근거로 사용하세요.
- reason에는 핵심 근거만 짧게 설명하세요.
- 신규 진입 또는 상승 방향 판단이면 BUY
- 하락 전환 또는 청산 방향 판단이면 SELL
- 애매하면 HOLD
