...
def create_mock_snapshot(record: dict) -> dict:
    ...
    snapshot = {
        ...
        "orderbook": {"buy_ratio": 0.5, "ask_prices": [], "ask_volumes": [], "bid_prices": [], "bid_volumes": []},
        "market_change": 0.0,
        "trade_strength": 0.0,
        "indicators": {},
        "analysis_candles": [],
    ...
    # 1. prompt_text가 있는 경우 (정규식 파싱)
    if old_prompt_text:
        m = re.search(r"시장 지수 변동률:\s*([-\d\.]+)%", old_prompt_text)
        if m: snapshot["market_change"] = float(m.group(1))
        
        m = re.search(r"체결강도:\s*([-\d\.]+)%", old_prompt_text)
        if m: snapshot["trade_strength"] = float(m.group(1))
        
        m = re.search(r"매수잔량비율:\s*([-\d\.]+)%", old_prompt_text)
        if m: snapshot["orderbook"]["buy_ratio"] = float(m.group(1)) / 100.0
    ...
    # 2. prompt_text가 없는 경우 (gate_detail 및 analysis에서 추출)
    else:
        # market_change 추출
        ...
        if m_ts: snapshot["trade_strength"] = float(m_ts.group(1))
        
        m_br = re.search(r"매수잔량비율[^\d]*(\d+)%", orderbook_str)
        if m_br: snapshot["orderbook"]["buy_ratio"] = float(m_br.group(1)) / 100.0
...
async def simulate_today(ticker: str):
    ...
    with log_file.open("r", encoding="utf-8") as f:
        for line in f:
            ...
            try:
                ...
                if record.get("event", "CYCLE") == "CYCLE" and record.get("gate_passed"):
    ...
    if not cycles:
        logger.warning("시뮬레이션을 실행할 수 있는 유효한 오늘자 사이클 로그(chart_paths 포함)가 없습니다.")
        return

    logger.info(f"총 {len(cycles)}개의 사이클 로그를 불러왔습니다. 시뮬레이션을 시작합니다...")
    ...
    for record in cycles:
        ...
        if not chart_paths:
            logger.warning(f"[{ts}] 차트 이미지가 존재하지 않아 이 사이클은 스킵합니다.")
            continue
        ...
        if ai_action in ("BUY", "SELL") and ai_confidence < config.ARBITER_MIN_CONFIDENCE:
            ai_action = "HOLD"
            
        logger.info(f"[{ts}] AI 결정: {ai_action} (신뢰도: {ai_confidence}) | {ai_reason}")
        ...
        if risk.has_position:
            ...
            if check.get("action") in ("STOP_LOSS", "TAKE_PROFIT", "TRAILING_STOP"):
                pnl = risk.close_position(current_price, close_reason=check.get("action"))
                logger.info(f"[{ts}] ⚡ 가상 청산 (리스크 관리: {check.get('action')}) -> 수익금: {pnl:,.0f}원")
                
        if ai_action == "BUY" and not risk.has_position:
            can_enter, enter_reason = risk.can_enter(current_price, virtual_total_assets)
            if can_enter:
                order_params = risk.calc_order_params(current_price, virtual_total_assets, 0.0)
                ...
                if qty > 0:
                    ...
                    logger.info(f"[{ts}] 🟢 가상 진입 (매수): {qty}주 @ {current_price:,}원")
            else:
