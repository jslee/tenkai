...
2. [핵심 수정] 매도 전략의 이원화 (Two-Tiered Exit Strategy):
   - **[A: 수익 보존형 매도 (Profit Taking - 중요)]** 
     * 현재 보유 포지션이 '수익 구간'일 경우: 단기 저항선(최근 고점 또는 볼린저밴드 상단) 터치, 혹은 1분봉상 주요 지지선(EMA 20 등)의 하향 이탈이 감지되면 '구조적 붕괴 여부와 상관없이' 즉시 SELL을 결정하여 수익을 확정하라. (수익을 반납하며 기다리지 마라.)
   - **[B: 추세 반전 매도 (Trend Reversal - 방어적)]** 
     * 현재 보유 포지션이 '본전 또는 손실 구간'일 경우: 반드시 대량의 거래량을 동반한 '구조적 추세 붕괴'(EMA 60 하향 돌파 또는 직전 저점 이탈)가 확인될 때만 SELL을 결정하라. (섣부른 손절로 인한 잦은 매매를 방지한다.)
